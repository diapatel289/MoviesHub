...
type QualityModelProps = ComponentProps & {
  fields: {
    Title: Field<string>;
...
const QualityModel =  (props: QualityModelProps): JSX.Element => {
  const [inputValue, setInputValue] = useState('');

  const handleSubmit = async(e: React.FormEvent) => {
    e.preventDefault();
    await postWithToast(
  'https://localhost:7107/api/metadata/add-quality',
  { name: inputValue },
...
export default withDatasourceCheck()<QualityModelProps>(QualityModel);
