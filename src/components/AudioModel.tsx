...
type AudioModelProps = ComponentProps & {
  fields: {
    Title: Field<string>;
...
const AudioModel = (props: AudioModelProps): JSX.Element => {
  const [inputValue, setInputValue] = useState('');

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    await postWithToast(
      'https://localhost:7107/api/metadata/add-audio',
      { name: inputValue },
...
export default withDatasourceCheck()<AudioModelProps>(AudioModel);
