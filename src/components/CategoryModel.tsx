...
type CategoryModelProps = ComponentProps & {
  fields: {
...
const CategoryModel = (props: CategoryModelProps): JSX.Element => {
  const [inputValue, setInputValue] = useState('');

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    await postWithToast(
      'https://localhost:7107/api/metadata/add-category',
      { name: inputValue },
...
export default withDatasourceCheck()<CategoryModelProps>(CategoryModel);
